...
"""Distillation script for Stable Diffusion XL for text2image"""
import gc
import argparse
import itertools
import logging
...
---
    """
    model_card = f"""
# text2image fine-tuning - {repo_id}

These are fine-tuned weights for {base_model}. The weights were fine-tuned on the {dataset_name} dataset. You can find some example images in the following. \n
{img_str}
"""
...
def cast_hook(unet, dicts, level = "block", layers = ["down", "mid", "up"], middle_truncate=False):
    if level == "block":
        for i in range(3):
            unet.down_blocks[i].register_forward_hook(getActivation(dicts,f'd{i}',True))
        unet.mid_block.register_forward_hook(getActivation(dicts,'m',False))
        for i in range(3):
            unet.up_blocks[i].register_forward_hook(getActivation(dicts,f'u{i}',False))
    elif level == "sub_block":
        if "down" in layers:
            for i in range(2):
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="Finetuning",
        help="The Name for the Wandb Experiment.",
    )
...
def main(args):
    ...
    vae = AutoencoderKL.from_pretrained(
        vae_path, subfolder="vae" if args.pretrained_vae_model_name_or_path is None else None, revision=args.revision #, use_safetensors=True
    )
    unet = UNet2DConditionModel.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="unet", revision=args.revision #, use_safetensors=True
    )
    # unet = UNet2DConditionModel.from_config('SDXL_modded_config.json')
    KD_teacher_unet = UNet2DConditionModel.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="unet", revision=args.revision #, use_safetensors=True
    )
    KD_teacher = {}
    KD_student= {}
    cast_hook(unet, KD_student, "sub_block",middle_truncate=True)
    cast_hook(KD_teacher_unet, KD_teacher, "sub_block")
    # We only train the unet
    vae.requires_grad_(False)
    KD_teacher_unet.requires_grad_(False)
    text_encoder_one.requires_grad_(False)
    text_encoder_two.requires_grad_(False)

    # For mixed precision training we cast all non-trainable weigths (vae and text_encoders) to half-precision
    ...
    if args.train_text_encoder:
        unet, text_encoder_one, text_encoder_two, optimizer, train_dataloader, lr_scheduler, KD_teacher_unet = accelerator.prepare(
            unet, text_encoder_one, text_encoder_two, optimizer, train_dataloader, lr_scheduler, KD_teacher_unet
        )
    else:
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("sdxl-distill", config=vars(args))
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if args.snr_gamma is None:
                    ...
                    for i in range(3):
                        for j in range(2):
                            loss_features+= F.mse_loss(KD_teacher[f"d{i}r{j}"], KD_student[f"d{i}r{j}"])
                            if (i!=0):
                                loss_features+= F.mse_loss(KD_teacher[f"d{i}a{j}"], KD_student[f"d{i}a{j}"])
                    loss_features=loss_features+F.mse_loss(KD_teacher['m'],KD_student['mr0'])
                    for i in range(3):
                        for j in range(3):
                            loss_features+= F.mse_loss(KD_teacher[f"u{i}r{j}"], KD_student[f"u{i}r{j}"])
                            if(i<2):
                                loss_features+= F.mse_loss(KD_teacher[f"u{i}a{j}"], KD_student[f"u{i}a{j}"])
                    ...
                    loss=loss_task+0.5*loss_KD+0.5*loss_features
                else:
